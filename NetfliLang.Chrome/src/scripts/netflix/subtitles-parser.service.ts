import { ISubtitles, ISubtitleOccurence } from 'src/shared/interfaces';
...
export class SubtitlesParserService {
  ...
  public parseSubtitles(ttmlDoc: Document): ISubtitles {
    ...
    const tickRate = Number(
      ttmlDoc.querySelector('tt').getAttribute('ttp:tickRate')
    );
    ...
    ttmlDoc.querySelectorAll('p').forEach((element, index, elements) => {
      ...
      const nextKey =
        index < elements.length - 1 ? elements[index + 1].textContent : null;
